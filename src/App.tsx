...
import {
  faHome, faBell, faPalette, faCube, faColumns, faThLarge,
...
const DEMO_TOOLS: ToolItem[] = [
  { id: 'overview', name: 'Overview', icon: faCube, description: 'OpenixWidgets Framework Overview' },
  { id: 'layout', name: 'Layout & Sidebar', icon: faColumns, description: 'Layout and Sidebar component demo' },
  { id: 'three-column', name: 'Three Column', icon: faThLarge, description: 'Three-column layout example' },
  { id: 'popup', name: 'Popup & Hooks', icon: faBell, description: 'Popup and usePopup Hook demo' },
  { id: 'theme', name: 'Theme System', icon: faPalette, description: 'Theme system: Switch themes and preview palettes' },
  { id: 'page-container', name: 'PageContainer', icon: faHome, description: 'PageContainer component demo' },
];

const OverviewDemo: React.FC = () => (
  <PageContainer title="OpenixWidgets Framework Overview" description="A modern UI component library based on React + TypeScript">
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      <div className="demo-card">
        <h3>Framework Components</h3>
        <ul style={{ lineHeight: 2, color: 'var(--color-text)' }}>
          <li><strong>CoreUI</strong> — Four core components: Layout, Sidebar, PageContainer, Popup</li>
          <li><strong>Themes</strong> — Complete theme system with ThemeProvider + CSS variables, 6 built-in themes</li>
          <li><strong>Hooks</strong> — Practical React Hooks like usePopup</li>
          <li><strong>Settings</strong> — Built-in settings panel (language, theme, sidebar)</li>
          <li><strong>i18n</strong> — Multi-language support (Simplified Chinese, Traditional Chinese, English, Japanese, Korean)</li>
        </ul>
      </div>
      <div className="demo-card">
        ...
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
          {['Catppuccin', 'Dracula', 'Atom One', 'Arduino', 'Yuzuki', 'Openix'].map((name) => (
            <span key={name} className="demo-badge">{name}</span>
          ))}
        </div>
      </div>
      <div className="demo-card">
...
const PageContainerDemo: React.FC = () => (
  <PageContainer title="PageContainer Component" description="PageContainer is a page-level container that provides title, description, and content area">
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      <div className="demo-card">
        <h3>Basic Usage</h3>
        <pre className="demo-code">{
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '14px' }}>
          <thead>
            <tr style={{ background: 'var(--color-surface1)' }}>
              ...
              <th style={{ padding: '10px 16px', textAlign: 'left', color: 'var(--color-text)' }}>Required</th>
...
const AppContent: React.FC = () => {
  ...
  const [sidebarCollapsed, setSidebarCollapsed] = useState(
    () => loadUISettings().sidebarCollapsed
  );
  ...
  const renderContent = () => {
    switch (activeTool) {
      ...
      case 'three-column': return <ThreeColumnDemo />;
      case 'popup': return <PopupDemo />;
      case 'theme': return <ThemeDemo />;
      case 'page-container': return <PageContainerDemo />;
      default: return <OverviewDemo />;
    }
  };
